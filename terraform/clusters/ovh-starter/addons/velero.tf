...
resource "kubernetes_secret_v1" "velero_cloud_credentials" {
  ...
  data = {
    cloud = <<-EOT
[default]
aws_access_key_id=${data.terraform_remote_state.cluster.outputs.object_storage_access_key}
aws_secret_access_key=${data.terraform_remote_state.cluster.outputs.object_storage_secret_key}
EOT
  }
